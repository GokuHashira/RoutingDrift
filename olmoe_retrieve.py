...
class OLMoESparseMoeBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        # This would show the graph break, need to make changes
        ...
        # Auxiliary load-balancing loss (OLMoE style)
        # --- GRAPH BREAK 3: .mean() on dynamic expert token counts ----------
        router_probs     = F.softmax(router_logits, dim=-1)              
        tokens_per_expert = (selected_experts == torch.arange(
            self.num_experts, device=x.device
        ).unsqueeze(0)).float().sum(0)                                    
...
def build_olmoe(device: str = "cuda", dtype: torch.dtype = torch.float16) -> OLMoEModel:
    model = OLMoEModel(OLMOE_CONFIG).to(device=device, dtype=dtype)
    model.eval()
    return model