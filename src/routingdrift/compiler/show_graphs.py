...
def main() -> None:
    """Render the standalone explanatory figures into OUTPUT_DIR."""
    ...
    plt.tight_layout()
    fig11.savefig(f"{OUTPUT_DIR}/graph11_graph_breaks.png", dpi=150,
    ...
    plt.tight_layout()
    fig12.savefig(f"{OUTPUT_DIR}/graph12_compile_modes.png", dpi=150,
                  bbox_inches="tight", facecolor="white")
    plt.close(fig12)
    print("saved graph12")
    ...
    fig_r.savefig(f"{OUTPUT_DIR}/routing_diagram.png", dpi=150,
                  bbox_inches="tight", facecolor="white")
    plt.close(fig_r)
