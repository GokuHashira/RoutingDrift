...
def _inductor_cpu_compiler_available() -> bool:
    # Inductor on CPU shells out to a C++ compiler. On Windows that is cl.exe (MSVC);
    # if it is not on PATH the entire compile sweep cannot run.
...
class CompileBenchmarker:
    ...
    def run(
        ...
        cpu_compile_ok = self.device != "cpu" or _inductor_cpu_compiler_available()
        if not cpu_compile_ok:
            console.print(
                "[red]▶ MSVC cl.exe not found on PATH — TorchInductor cannot codegen on CPU/Windows.[/red]\n"
                "[red]  Install 'Build Tools for Visual Studio' (Desktop development with C++) and run\n"
                "  vcvars64.bat before re-running. Skipping compile modes.[/red]"
            )
